...
def add(course_student: CourseStudent, user=Depends(get_current_user)) -> dict:
...
def add_all_teacher_courses(data: BulkCourseStudentInput, user=Depends(get_current_user)) -> dict:
    return add_bulk_course_students_to_db(course_students=data)


@router.post("/display", response_model=dict, summary="Display Course")
def display(data: CourseStudent, user=Depends(get_current_user)) -> dict:
...
def fetch_course_student_by_course_id(data: CourseIdInput, user=Depends(get_current_user)) -> dict:
...
def add_students_to_course(data: BulkCourseStudentInput, user=Depends(get_current_user)) -> dict:
